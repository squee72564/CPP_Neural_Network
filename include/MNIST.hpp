        ...
        uint32_t& image_size)
{
    ...
    if(file.is_open()) {
        ...
        std::vector<std::vector<unsigned char> > _dataset(number_of_images, std::vector<unsigned char>(image_size, 0));

        for(int i = 0; i < number_of_images; i++) {
            file.read(reinterpret_cast<char *>(_dataset[i].data()), image_size);
        }
        return _dataset;
    } else {
        ...
        uint32_t& number_of_labels) 
{
    ...
    if(file.is_open()) {
        ...
        std::vector<unsigned char> _dataset(number_of_labels, 0);
